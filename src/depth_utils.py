...
def compute_surfnorm_from_depth(depth):
...
def colorized_depth(path):
    with open(path, 'rb') as f:
        ...
        return compute_surfnorm_from_depth(img)
...
def colorized_surfnorm_fukuoka(img_path):
    # depth_img = Image.open(img_path, 'r')
    depth_img = np.genfromtxt(str(img_path)[:str(img_path).find(".png")]+".txt", dtype=float)
    depth_img = np.nan_to_num(depth_img)
    depth_img = depth_img * 1000

    img = fast_depth_interpolation(depth_img)

    # img = compute_surfnorm_from_depth(img)

    pcl = depth_to_pcl(img)
...
def colorized_surfnorm_nyu(depth_img):
    depth_img = depth_img * 1000

    # img = fast_depth_interpolation(depth_img)

    pcl = depth_to_pcl(depth_img)
